...
const constructHistory = async(courses) => {
    ...
    courses.forEach(curcourseHistory => {
        ...
        curcourseHistory.batches.forEach(curBatchHistory => {
            let batchObj = {};
            const thisBatch = thiscourse.batch.find(curBatch => curBatchHistory.batch == curBatch._id);
            ...
            curBatchHistory.subjects.forEach(curSubjectHistory => {
                let subjectObj = {};
                const thisSubject = thisBatch.subjects.find(curSubject => curSubjectHistory == curSubject._id);

...
router.post('/getStudentsForExam', auth, async (req, res) => {
    try {
        ...
        const students = await Student.find({course: req.body.course, batch: req.body.batch, $or: [{status: "0"}, {status: "1"}], subjects: {$all: [{_id: req.body.subject}]}})
...
router.post('/getExams', auth, async (req, res) => {
    ...
    const exam = await Exam.find({course: req.body.course, batch: req.body.batch, subject: req.body.subject, date: regExp});
...
router.post('/getExamsPerformance', auth, async (req, res) => {
    try {
        ...
        if(req.body.subject == '0') {
            searchData = {date: date, marks: {$elemMatch: {student: mongoose.Types.ObjectId(req.body.student)}}}
        } else {
            searchData = {date: date, subject: req.body.subject, marks: {$elemMatch: {student: mongoose.Types.ObjectId(req.body.student)}}}
        }

...
router.post('/getStudentsForAttendance', auth, async (req, res) => {
    try {
        ...
        let attendance = await Attendance.findOne({date: req.body.date, course: req.body.course, batch: req.body.batch, subject: req.body.subject});
        ...
        const students = await Student.find({course: req.body.course, batch: req.body.batch, $or: [{status: "0"}, {status: "1"}], subjects: {$all: [{_id: req.body.subject}]}}, {_id: 1, name: 1})
...
router.post('/getAttendance', auth, async (req, res) => {
    try {
        ...
        if(req.body.subject == '0') {
            searchData = {date: date, attendance: {$all: [{$elemMatch: {student: mongoose.Types.ObjectId(req.body.student)}}]}}
        } else {
            searchData = {date: date, subject: req.body.subject, attendance: {$all: [{$elemMatch: {student: mongoose.Types.ObjectId(req.body.student)}}]}}
        }
